...
/**
 * 客户端 bundle 的模块表注册 id —— 必须是包名，
 * 与 deepseek-harness `ClientModuleRegistry` 扫描产物一致
 * （graph row id = package.json 的 name；路由 /plugins/<id>/client.js 允许 scope 斜杠）。
 */
const CLIENT_ID = '@dsh-better/session-drawer'
 ...
 * 同时把 host 期望的闭包工厂包（window.__ModuleLoader__.load）统一在本插件内完成：
...
function inlineClientCss(): Plugin {
  return {
    name: '@dsh-better/session-drawer-inline-css',
    apply: 'build',
    generateBundle(_outputOptions, bundle) {
      ...
      // 先为外置依赖（react 等）经注入 require 解析的 CJS 产物补上 module/exports 变量，
      // 再按宿主 ClientModuleRegistry 期望把整段注册到 window.__ModuleLoader__。
      // 注意：vite 把抽取的 .css 走单独管线写盘、不进入 rollup bundle，故 CSS 在
      // closeBundle 里读盘再注入。
      chunk.code =
...
/**
 * @dsh-better/session-drawer 构建配置：产出 host/client 双半区产物。
 *
 * - host 半区：src/index.ts → lib/index.js（Node ESM，对应 exports "."）
 * - client 半区：src/client/index.ts → lib/client.js（浏览器 CJS 工厂，对应 exports "./client"）
 *
 * external 清单包含三类：
 *   - react / react-dom：peerDependencies，由宿主浏览器壳提供
 *   - @deepseek-ai/*：宿主 cordis / dsh-* 运行时，由宿主进程/浏览器壳提供
 *   - node:* / node 内置模块：host 半区用的 Node API
 *
 * clsx 不外置：它只是普通 npm 库，直接打进 client bundle（宿主模块表不提供它）。
 *
...
const clientConfig: UserConfig = {
  build: {
    ...
    // 里对产物文本的整体包裹引用到的 `module` `exports` 变量名保持确定。包裹与 CSS
    // 内联均由 inlineClientCss 插件完成。
    minify: false,
    rollupOptions: {
      external,
      output: {
...
export default defineConfig(({ mode }) => (mode === 'client' ? clientConfig : hostConfig))
